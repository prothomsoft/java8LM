...
public class MultiRunner {
	...
	public static void runMultiThreaded(int times, Runnable block) {
	    //for (int i = 0; i < times; i++) pool.submit(block);
	    
	    IntStream.rangeClosed(1, times).mapToObj(integer -> block).forEach(pool::submit);
	}
	...
	public static void runMultiThreaded(Iterable<Runnable> blocks) {
	    ...
	    blocks.forEach(block -> pool.submit(block));
	    
	    /*blocks.forEach(new Consumer<Runnable>() {
            @Override
